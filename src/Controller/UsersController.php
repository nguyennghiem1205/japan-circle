...
class UsersController extends AppController
{
    ...
    function ajax_forgot()
    {
        if($this->request->is('ajax')) 
        {
            ...
            {
                $this->Flash->error(__('Email is not exist in the system, please try again'));
                $this->ajax['status'] = AJAX_STATUS_ERROR;
                $this->ajax['redirect'] = $this->request->webroot;
            }
        }
     ...
     * @param null $idMember
     */
    public function detailApi($idMember = null) {
        $key = $this->request->query('key');
        $languageSet = $this->request->query('language');
        switch ($languageSet) {
            case 'jpn':
                $fieldLanguage = 'ja_JP';
                break;
        ...
        I18n::locale($fieldLanguage);
        Configure::load('constant', 'default', false);
        if ($key != KEY_API || !$idMember) {
            throw new NotFoundException();
        }
        $this->loadModel('Members');
        $member = $this->Members->find()->where(['Members.id' => $idMember])
            ->contain(['Productions', 'CompanyInquiries', 'TypeCompanies', 'TypeBusiness'])
            ->first();
        if (!$member) {
            echo '';
            exit;
        }
        $fieldsNotGet = array('id', 'password', 'code', 'modified', 'created', 'com_view', 'com_send_email', 'com_rate', 'type_company_id', 'type_business_id', 'need_id', 'cooperation_id');
        $dataMember = array();
        foreach ($member->toArray() as $field => $value) {
            if (in_array($field, $fieldsNotGet)) {
                continue;
            }
            switch ($field) {
                case 'com_lang_chosen':
                    $languagesChosen = explode(',', $value);
                    $languages = Configure::read('Member.language');
                    $chosen = array();
                    foreach ($languagesChosen as $lang) {
                        $chosen[] = $languages[$lang];
                    }
                    $value = implode(', ', $chosen);
                    break;
                case 'gender':
                    $genderSetting = Configure::read('Member.gender');
                    $value = $genderSetting[$value];
                    break;
                case 'com_country':
                    $countries = Configure::read('Member.country');
                    $value = $countries[$value];
                    break;
                case 'com_level':
                    $levels = Configure::read('Member.type');
                    $value = $levels[$value];
                    break;
                case 'com_status':
                    $status = Configure::read('Member.status');
                    $value = $status[$value];
                    break;
                case 'image':
                case 'image_product_1':
                case 'image_product_2':
                case 'image_product_3':
                case 'image_product_4':
                case 'image_product_5':
                    if ($value) {
                        $value = Router::url('/upload/Members/'.$value, true);
                    } else {
                        $value = Router::url('/img/nophoto.png', true);
                    }
                    break;
                case 'company_inquiries':
                case 'productions':
                    $dataReturn = $valueReturn = array();
                    $modelPath = 'CompanyInquiries';
                    if ($field == 'productions') {
                        $modelPath = 'Productions';
                    }
                    foreach ($value as $dataSub) {
                        if ($field == 'company_inquiries') {
                            $url = Core::generateUrl('companyInquiryIndex', [
                                'name' =>  $dataSub['title'],
                                'id' => $dataSub['id']
                            ], true);
                        } else {
                            $url = Router::url(['controller' => 'Pages', 'action' => 'detailMember', $idMember], true);
                        }
                        foreach ($dataSub as $keySub => $valueSub) {
                            switch ($keySub) {
                                case 'image':
                                    if ($valueSub) {
                                        $valueSub = Router::url('/upload/'.$modelPath.'/'.$valueSub, true);
                                    } else {
                                        $valueSub = Router::url('/img/nophoto.png', true);
                                    }
                                    break;
                            }
                            $dataReturn[$keySub] = $valueSub;
                        }
                        $dataReturn['url'] = $url;
                        $valueReturn[] = $dataReturn;
                    }
                    $value = $valueReturn;
                    break;

            }
            $dataMember[$field] = $value;
        }
