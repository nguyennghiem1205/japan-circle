    ...
    <div class="col-xs-12 col-sm-5 col-md-5 col-lg-8">
        <ul id="sparks" class="">
            <li class="sparks-info">
                <?= $this->Html->link(__('New News Category'), ['action' => 'add'], ['class' => 'btn btn-primary']) ?>
            </li>
        </ul>
    </div>
</div>
    ...
    <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
        ...
        <div class="jarviswidget jarviswidget-color-darken" id="wid-id-0" data-widget-colorbutton="false" data-widget-deletebutton="false" data-widget-editbutton="true">
            <header>
                ...
                <h2><?= __('List of News Category')?></h2>
            </header>
            <!-- widget div-->
            <div>
                <!-- widget edit box -->
                    ...
                    <tbody>
                        <?php foreach ($newsCategories as $newsCategory): ?>
                            <tr>
                                ...
                                <td><?= h($newsCategory->name_vie) ?></td>
                                <td><?= h($newsCategory->name_eng) ?></td>
                                <td><?= h($newsCategory->name_jpn) ?></td>
                                <td><?= h($newsCategory->created) ?></td>
                                <td><?= h($newsCategory->modified) ?></td>
                                <td>
                                    <?= $this->Html->link('<i class="fa fa-eye"></i> ', array('action' => 'view', $newsCategory->id), array('title' => __('View'),'escape' => false, 'class' => 'btn btn-sm btn-default') );?>
                                    <?= $this->Html->link('<i class="fa fa-edit"></i> ', array('action' => 'edit', $newsCategory->id), array('title' => __('Edit'),'escape' => false, 'class' => 'btn btn-sm btn-default') ); ?>
                                    <?php if($this->CurUser->group != GROUP_MODERATOR):?>
                                    <?= $this->Form->postLink('<i class="fa fa-trash-o"></i> ', ['action' => 'delete', $newsCategory->id], ['data-action' => 'deleteLin', 'data-delete-msg' => __('Are you sure you want to delete # {0}?', $newsCategory->id), 'class' => 'btn btn-sm btn-default deleteLin', 'escape' => false]) ?>
                                    <?php endif ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
