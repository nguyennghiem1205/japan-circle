                ...
                <div class="widget-body no-padding">
                    ...
                    <thead>
                        <tr>
                                            ...
                                            <th class="hasinput"><input type="text" class="form-control" placeholder="<?= __('Filter file')?>" /></th>
                    ...
                    <tbody>
                        <?php foreach ($events as $event): ?>
                            <tr>
                                ...
                                <td><?= h($event->title_vie) ?></td>
                                <td><?= h($event->title_eng) ?></td>
                                <td><?= h($event->title_jpn) ?></td>
                                <td><?= $this->Core->image('Events/'.$event->image, 60, 60) ?></td>
                                <td><?= h($event->file) ?></td>
                                <td><?= $this->Time->format($event->created, 'dd/MM/Y') ?></td>
                                <td>
                                    <?= $this->Html->link('<i class="fa fa-eye"></i> ', array('action' => 'view', $event->id), array('title' => __('View'),'escape' => false, 'class' => 'btn btn-sm btn-default') );?>
                                    <?= $this->Html->link('<i class="fa fa-edit"></i> ', array('action' => 'edit', $event->id), array('title' => __('Edit'),'escape' => false, 'class' => 'btn btn-sm btn-default') ); ?>
                                    <?php if($this->CurUser->group != GROUP_MODERATOR):?>
                                    <?= $this->Form->postLink('<i class="fa fa-trash-o"></i> ', ['action' => 'delete', $event->id], ['data-action' => 'deleteLin', 'data-delete-msg' => __('Are you sure you want to delete # {0}?', $event->id), 'class' => 'btn btn-sm btn-default deleteLin', 'escape' => false]) ?>
                                    <?php endif ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
