...
<div class="row">
    <div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
        <h1 class="page-title txt-color-blueDark">
            ...
            <?= __('<%= $pluralHumanName %>');?>
            <span>&gt;
                <?= $this->Html->link(__('List of <%= $singularHumanName %>'), ['action' => 'index'])?>
            </span>
        </h1>
    </div>
</div>
...
%>
                            <?= $this->Form->input('<%= $field %>', ['placeholder' => __('<%= $field %>'), 'options' => $<%= $keyFields[$field] %>, 'empty' => true]); ?>
<%
                                } else {
%>
                            <?= $this->Form->input('<%= $field %>', ['placeholder' => __('<%= $field %>'), 'options' => $<%= $keyFields[$field] %>]) ?>
<%
                                }
                                continue;
                            }
...
%>
                            <?= $this->Form->input('<%= $field %>', ['placeholder' => __('<%= $field %>'), 'empty' => true, 'default' => '']) ?>
<%
                                } else {
%>
                            <?= $this->Form->input('<%= $field %>', ['placeholder' => __('<%= $field %>')]) ?>
<%
                                }
                            }
%>
